...
当 AI 返回的工具调用即将执行时，后端在工具执行前向 SSE 流发送 `tool_call_start` 事件。
...
当 auto-tool 执行完毕（成功或失败）后，后端向 SSE 流发送 `tool_result` 事件。
...
前端 SSE 客户端处理 `tool_call_start` 和 `tool_result` 两种新事件类型。
...
- **THEN** 前端忽略该事件，不影响其他事件的处理

### Requirement: 前端显示工具调用状态卡片

前端为每个工具调用渲染为独立的折叠式状态卡片，展示调用过程和结果。
