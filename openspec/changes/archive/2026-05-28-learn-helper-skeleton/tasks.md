...
- [x] 2.2 安装依赖（chi、modernc.org/sqlite、google/uuid）
...
- [x] 6.1 启动 Go 后端（`go run cmd/server/main.go`）
- [x] 6.2 启动前端（`pnpm dev`）
- [x] 6.3 浏览器访问 http://localhost:3000，验证页面正常渲染
- [x] 6.4 验证 API 响应（知识图谱数据加载正常）
- [x] 6.5 配置 AI API Key 后测试 AI 对话（可选）— 已手动验证 UI 正常