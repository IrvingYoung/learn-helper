...
func WikiTools() []Tool {
	return []Tool{
		...
		// shows as counts (e.g. "8 反链") into actual lists, so the AI can
		// follow links/backlinks/children without resorting to full-table
		// search_pages scans.
		{
			Name:        "list_backlinks",
			Description: "列出引用了该页面的所有页（即该页的反链来源）。比 search_pages 精确——直接读 wiki_pages.backlinks。重构/重命名/合并前用这个看连带影响。",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"page_id": map[string]any{"type": "integer", "description": "目标页 ID"},
				},
				"required": []string{"page_id"},
			},
		},
		{
			Name:        "list_links",
			Description: "列出该页面引用了哪些页（出链）。读 wiki_pages.links。看一页都引到了哪些相关知识时用。",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"page_id": map[string]any{"type": "integer", "description": "源页 ID"},
				},
				"required": []string{"page_id"},
			},
		},
		{
			Name:        "list_children",
			Description: "列出指定父页面下的直接子页（或递归展开子树）。比知识地图的全局视图更精准——只看一个分类的局部。depth=1 只返回直接子页，depth>1 递归展开。",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"parent_id": map[string]any{"type": "integer", "description": "父页 ID;0 或留空表示顶层"},
					"depth":     map[string]any{"type": "integer", "description": "递归深度,默认 1;最大 5"},
				},
		...
		},
		{
			...
			Description: "找出指向不存在标题的 [[X]] 链接（死链）。不传 page_id 时扫全库；传了只扫该页。用来在重命名/移动后清理失效引用。",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"page_id": map[string]any{"type": "integer", "description": "可选,只检查这一页;留空扫全库"},
				},
			},
		},
...
- ` + "`list_backlinks`" + ` / ` + "`list_links`" + ` / ` + "`list_children`" + ` / ` + "`find_broken_links`" + ` 结构化导航（反链 / 出链 / 子页 / 死链）
