...
import { CronRun, listCronRuns, getCronRun } from "../lib/api-cron";
...
export function CronRunHistory({ taskId }: Props) {
  const { data: runs, mutate } = useSWR<CronRun[]>(
    `cron-runs-${taskId}`,
    () => listCronRuns(taskId, 20),
  ...
  return (
    <div>
      <h3 className="text-lg font-medium mb-3">运行历史</h3>
      {!runs ? (
        <p className="text-sm text-gray-500">加载中...</p>
      ) : runs.length === 0 ? (
        <p className="text-sm text-gray-500">暂无运行记录</p>
      ) : (
      ...
      {selectedRun && <CronRunDetailDialog run={selectedRun} onClose={() => setSelectedRun(null)} />}
...
function CronRunDetailDialog({ run, onClose }: { run: CronRun; onClose: () => void }) {
  return (
    <div
